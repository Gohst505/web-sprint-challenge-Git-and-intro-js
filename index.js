...
function getArtistByIndex(artist, number) {
  artist = artists[number].name;
  return `the artist at index ${number} is ${artist}`;
}
...
function removeArtist(artist, indexNum,) {
  const removeArray = [];
...
function lotsOfArt(artists) {
  ...
  for(let i = 0; i < artists.length; i++){
    if(artists[i].paintings > 100){
      paintingCount.push(artists[i].name);
    }
  }
...
function artistInfo(artist, artistName){
  ...
  for(let i = 0; i < artist.length; i++){
    if (artistName === artist[i].name){
      artistBio.push(artist[i].bio);
    }
  }
  return artistBio.toString();
}
...
function artistByCountry(artist, nationality){
  ...
  for(let i = 0; i < artist.length; i++){
    if(nationality === artist[i].nationality){
      artistNationality.push(artist[i].name);
    }
  }
